...
module.exports = function(dir,env){
  ...
  var ret = { 
    ...
    srcFolder : [], // default to project/src
    rootFolder : dir,
    deps : []
  };
  ...
  if(fs.existsSync(packageFile)){
    try{
      ...
      if(packageJson.leoFiles && util.isArray(packageJson.leoFiles) ){
      	ret.files = packageJson.leoFiles.map(function(f) {
          var file = path.join(dir, f);
          return file;
        });

        ret.files.forEach(function(f) {
          ret.srcFolder.push(path.dirname(f));
        });
  ...
  if(ret.files.length === 0){
    ...
    if (fs.existsSync(srcFolder)) {
      ret.srcFolder.push(srcFolder);
      scan(srcFolder, MAX_SCAN_DEPTH);
    } else {
      // no source folder found, scan project dir but not recursivly
      ret.srcFolder.push(dir);
      scan(dir,1);
    }
  }
  ...
  if(ret.name === '.' && ret.main.length > 0){
    ret.name = path.basename(ret.main[0], path.extname(ret.main[0]));
  }

  if (ret.main.length === 0 && ret.files.length === 0) {
    return null;
  }
